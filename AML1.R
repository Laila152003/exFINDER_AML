...
all.samples<-AML
healthy_samples<- subset(x=all.samples, subset= orig.ident %in% c('BM1', 'BM2', 'BM3', 'BM4','BM5.34p','BM5.34p38n' ))
AML_samples<- subset(x=all.samples, subset= orig.ident %in% c('AML1012.D0', 'AML210A.D0', 'AML419A.D0', 'AML916.D0', 'AML921A.D0', 'AML314.D0', 'AML314.D31', 'AML371.D0', 'AML371.D34', 'AML475.D0', 'AML475.D29', 'AML722B.D0', 'AML722B.D49', 'AML870.D0', ' AML870.D14', ' AML997.D0','AML997.D35', 'AML329.D0','AML329.D20', 'AML329.D37', ' AML420B.D0', 'AML420B.D14 ', 'AML420B.D35', 'AML556.D0', 'AML556.D15', ' AML556.D31', 'AML328.D0','AML328.D29 ', 'AML328.D113', 'AML328.D171', 'AML707B.D0', 'AML707B.D18', 'AML707B.D41', 'AML707B.D97', 'AML707B.D113') )
malignant_cells<- subset(x=AML_samples, subset = PredictionRefined == 'malignant')
DimPlot(malignant_cells, reduction = "umap",  group.by = 'CellType', label= TRUE, label.size=3) +
  ggtitle('Cell Types Malignant')
healthy_malignant<- subset(x=all.samples, subset= orig.ident %in% c('BM1', 'BM2', 'BM3', 'BM4','BM5.34p','BM5.34p38n') | PredictionRefined == 'malignant')
...
DimPlot(AML_samples, reduction = "umap",  group.by = 'PredictionRefined', label= TRUE, label.size=3) +
  ggtitle('AML_samples state')

healthy_samples<- ScaleData(healthy_samples)
healthy_samples<-FindVariableFeatures(healthy_samples)  #I want to detect the marker genes that differentiate between malignant and nonmalignant cells in AML sample
healthy_samples <- RunPCA(healthy_samples, assay = "RNA", verbose = FALSE)
healthy_samples<- RunUMAP(healthy_samples, dims = 1:20)
DimPlot(healthy_samples, reduction = "umap",  group.by = 'PredictionRefined', label= TRUE, label.size=3) +
  ggtitle('healthy_samples state')

healthy_malignant<-ScaleData(healthy_malignant)
healthy_malignant<-FindVariableFeatures(healthy_malignant)  #I want to detect the marker genes that differentiate between malignant and nonmalignant cells in AML sample
healthy_malignant <- RunPCA(healthy_malignant, assay = 'RNA', verbose = FALSE)
healthy_malignant<- RunUMAP(healthy_malignant, dims = 1:20)
DimPlot(healthy_malignant, reduction = "umap",  group.by = 'PredictionRefined', label= TRUE, label.size=3) +
...
healthy_malignant.markers <- FindAllMarkers(healthy_malignant)
healthy_malignant.markers<- healthy_malignant.markers %>%
  group_by(cluster) %>%
  dplyr::filter(avg_log2FC > 1 & p_val_adj< 0.01)
healthy_malignant.markers<- healthy_malignant.markers%>% arrange(avg_log2FC,)



DoHeatmap(healthy_malignant, features= target.gene, group.by= 'PredictionRefined') + ggtitle ('Healthy vs. Malignant')
FeaturePlot(AML_samples, features = target.gene)
FeaturePlot(Prog.Patient, features = target.gene)

AML_samples@assays$RNA$
